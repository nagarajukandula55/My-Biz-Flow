...
/**
 * SC Profile — a sub-SC (sub-service-centre) / outsourced repair partner
 * that THIS Service Centre partner works with (e.g. a specialist
 * screen-repair shop a job gets outsourced to, or a spare-parts supplier).
 * Ported from AN-CRM's src/models/VendorProfile.ts; renamed from "Vendor
 * Profile" to "SC Profile" because "vendor" is AN-CRM-specific language
 * that doesn't fit a multi-vertical platform — see project-level notes.
 *
 ...
 * itself — it logs in, owns a subscription/plan, and gets an id (e.g.
 * SC#### for the Service Centre partner type) used for the partner's own
 * login/invoices/support. AN-CRM's VendorProfile is a different
 * relationship entirely: it's a business *that this tenant's Service
 * Centre deals with* (a sub-contractor or supplier), scoped under a
 * single partner, with no login of its own — exactly the shape of every
 * other BusinessRecord-backed catalog in this module (Brands, Models,
 * Solutions, Fault/Symptom Codes), not a new tenant. So it belongs here,
 * under service-centre/sc-profile/, following the exact same
 * Column/FormFieldDef/DataTable/RecordForm/RecordDetail convention as
 * scBrandColumns etc., rather than extending Partner or Prisma.
 *
 * parentScId supports a simple one-level-deep sub-SC hierarchy (e.g. a
 * regional SC with local branches), same idea as AN-CRM's parentVendorId
 * — resolved by id against this same catalog.
 */
...
export const scProfileColumns: Column[] = [
  ...
  { key: "parentScId", label: "Parent SC", type: "text" },
...
export const scProfileRows: Row[] = [
  {
    id: "SVP-001",
...
export const scProfileFormFields: FormFieldDef[] = [
  { key: "id", label: "SC Code", type: "text", required: false, placeholder: "Auto-generated if left empty" },
  { key: "businessName", label: "SC Business Name", type: "text", required: true },
  { key: "onboardingStatus", label: "Onboarding Status", type: "select", required: true, options: [...ONBOARDING_STATUSES] },
  { key: "parentScId", label: "Parent SC Code (for a sub-SC / branch)", type: "text", required: false },
...
export function getScProfileRecord(recordId: string): Row {
  return scProfileRows.find((r) => String(r["id"]) === recordId) ?? scProfileRows[0];
}

export function getScProfileDetailFields(record: Row): RecordField[] {
  const r = record;
  return [
    { label: "SC Code", value: r["id"], type: "text" },
    { label: "SC Business Name", value: r["businessName"], type: "text" },
    ...
    { label: "Parent SC", value: r["parentScId"], type: "text" },
...
export function getScProfileTimeline(record: Row): TimelineEntry[] {
  return [
    { id: "t1", label: `SC "${record["businessName"]}" applied for onboarding`, timestamp: "2026-07-01T09:00:00", actor: "Partner Admin" },
    { id: "t2", label: `Onboarding status set to ${record["onboardingStatus"]}`, timestamp: "2026-07-05T11:00:00", actor: "Partner Admin" },
  ];
}

export const scProfileRelated: RelatedRecord[] = [];

/** Dropdown options for use elsewhere (e.g. BOM's Supplier / Vendor Ref field), filtered to Active. */
export function getScProfileOptions(): { value: string; label: string }[] {
  return scProfileRows
