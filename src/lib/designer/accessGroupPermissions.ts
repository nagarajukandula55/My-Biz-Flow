/**
 * Registry-dependent helper for the Access Group page-permission matrix.
 * Split out from accessGroupsData.ts for the same reason renderTemplate.ts
 * is split from documentTemplates.ts: getRegisteredPages()/registerAll.ts
 * pull in node:fs transitively — must only ever be imported from Server
 * Components (the New/Edit access-group pages), never from anything
 * client-bundled.
 */
