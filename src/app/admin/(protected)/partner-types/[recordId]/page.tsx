...
import { getPartnerType, PLAN_TIERS } from "@/lib/designer/partnerTypesData";
import { getAssignablePagesByModule } from "@/lib/designer/accessGroupPermissions";
import { listPlans } from "@/lib/plansData";
import { DeletePartnerTypeButton } from "./DeletePartnerTypeButton";

export const dynamic = "force-dynamic";

registerPage({
  id: "platform.partner-types.detail",
  moduleSlug: "platform",
  title: "Partner Types — Detail",
  path: "/admin/partner-types/[recordId]",
  kind: "detail",
  superAdminOnly: true,
  customizableRegions: [{ key: "field-grid", label: "Detail field grid" }],
  explanation: "Read-only detail view of a single Partner Type, with Edit and Delete actions.",
  sourceFile: "src/app/admin/(protected)/partner-types/[recordId]/page.tsx",
});

const TIER_LABEL: Record<string, string> = { basic: "Basic", pro: "Pro", ultimate: "Ultimate" };

export default async function PartnerTypeDetailPage({ params }: { params: { recordId: string } }) {
  const type = await getPartnerType(params.recordId);
  if (!type) notFound();
        ...
        <div className="p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link href="/admin/partner-types" className="btn-outline">
                &larr; Back
              </Link>
              <Link href={`/admin/partner-types/${type.id}/edit`} className="btn-outline">
                Edit
              </Link>
              <DeletePartnerTypeButton id={type.id} />
            </div>
          </div>
          ...
          <div className="mt-6 rounded-lg border border-border bg-bg-raised p-5">
            ...
            <p className="mt-1 text-xs text-text-muted">
              The real Plans (billing/pricing) this Partner Type offers at signup — separate from the page-tier
              breakdown above.
            </p>
