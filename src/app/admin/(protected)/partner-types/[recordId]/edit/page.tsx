...
import { updatePartnerTypeAction } from "../../actions";

export const dynamic = "force-dynamic";

registerPage({
  id: "platform.partner-types.edit",
  moduleSlug: "platform",
  title: "Partner Types — Edit",
  path: "/admin/partner-types/[recordId]/edit",
  kind: "form",
  superAdminOnly: true,
  customizableRegions: [{ key: "form-fields", label: "Form fields" }],
  explanation: "Edit form for an existing Partner Type, writing to the PartnerType Prisma table.",
  sourceFile: "src/app/admin/(protected)/partner-types/[recordId]/edit/page.tsx",
});

export default async function EditPartnerTypePage({ params }: { params: { recordId: string } }) {
  const type = await getPartnerType(params.recordId);
  if (!type) notFound();
  const roles = await listRoles();
  const pagesByModule = getAssignablePagesByModule();
  const plans = await listPlans();
  const updateAction = updatePartnerTypeAction.bind(null, type.id);

  return (
    <SuperAdminGate>
      <div className="mbf-page">
        <div className="border-b border-border bg-bg-raised px-6 py-4">
          <h1 className="font-display text-lg font-bold text-text">Edit Partner Type</h1>
        </div>
        <div className="p-6">
            ...
            <div className="max-w-2xl space-y-4">
              ...
              <div>
                ...
                <p className="mb-2 text-xs text-text-muted">
                  Which of the 3 real Plans (billing/pricing) this Partner Type offers at signup — separate from
              ...
              <div>
                <label className="flex items-center gap-2 text-sm text-text">
                  <input
                    ...
                    className="h-4 w-4 accent-accent"
                  />
                  Require Super Admin approval before assigning a Partner ID
                </label>
