        ...
        {(showGoogle || (showTelegram && telegramBotUsername)) && (
          <div className="mt-6 flex flex-col gap-3">
            ...
            <div className="flex items-center justify-center gap-4">
              {showGoogle && (
                // Plain <a>, not next/link -- this route 302s to
                // accounts.google.com, a cross-origin redirect. next/link
                // intercepts the click as a client-side RSC fetch, and
                // browsers block a cross-origin redirect on a fetch (CORS),
                // which silently broke this button. A real <a> forces an
                // ordinary top-level navigation, which follows redirects fine.
                <a
                  href="/api/auth/google"
                  title="Sign in with Google"
                  aria-label="Sign in with Google"
                  className="flex h-11 w-11 items-center justify-center rounded-full border border-border bg-bg hover:bg-bg-sunken"
                >
