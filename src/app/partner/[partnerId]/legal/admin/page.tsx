...
registerPage({
  ...
  moduleSlug: "legal",
  title: "Legal — Admin",
  path: "/partner/[partnerId]/legal/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/legal/admin/page.tsx",
});

export default async function LegalAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("legal");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Legal"} · Admin`}>
      <SuperAdminGate>
        <div>
