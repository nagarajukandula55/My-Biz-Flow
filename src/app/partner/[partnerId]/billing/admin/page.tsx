...
registerPage({
  ...
  moduleSlug: "billing",
  title: "Billing — Admin",
  path: "/partner/[partnerId]/billing/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/billing/admin/page.tsx",
});

export default async function BillingAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("billing");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Billing"} · Admin`}>
      <SuperAdminGate>
        <div>
