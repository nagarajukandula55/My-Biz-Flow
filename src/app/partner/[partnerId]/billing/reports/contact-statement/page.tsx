...
registerPage({
  ...
  path: "/partner/[partnerId]/billing/reports/contact-statement",
  kind: "list",
  ...
  sourceFile: "src/app/partner/[partnerId]/billing/reports/contact-statement/page.tsx",
});

export const dynamic = "force-dynamic";

export default async function ContactStatementIndexPage({ params }: { params: { partnerId: string } }) {
  const contacts = await listBusinessRecords(params.partnerId, "billing-contacts");
      ...
      <div>
        ...
        <div className="mt-4 divide-y divide-border rounded-lg border border-border bg-bg-raised">
          ...
          {contacts.map((c) => (
            <Link
              key={String(c["id"])}
              href={`/partner/${params.partnerId}/billing/reports/contact-statement/${c["id"]}`}
