...
registerPage({
  ...
  path: "/partner/[partnerId]/billing/reports/contact-statement/[contactId]",
  kind: "dashboard",
  ...
  sourceFile: "src/app/partner/[partnerId]/billing/reports/contact-statement/[contactId]/page.tsx",
});
...
export default async function ContactStatementPage({
  ...
  const contact = await getBusinessRecord(params.partnerId, "billing-contacts", params.contactId);
  if (!contact) notFound();
  ...
  return (
    <AppShell topbarTitle={`Statement — ${contactName}`}>
      <div>
        ...
        <div className="mt-4">
          <Link href={`/partner/${params.partnerId}/billing/reports/contact-statement`} className="btn-outline">
