...
registerPage({
  ...
  path: "/partner/[partnerId]/billing/recurring",
  kind: "list",
  ...
  sourceFile: "src/app/partner/[partnerId]/billing/recurring/page.tsx",
});
...
export default async function RecurringInvoicesPage({ params }: { params: { partnerId: string } }) {
  ...
  return (
    <AppShell
      ...
      topbarActions={
        <Link href={`/partner/${params.partnerId}/billing/recurring/new`} className="btn-accent">
    ...
    >
      <div>
        <div className="mt-2">
          <RecurringClientTable partnerId={params.partnerId} columns={columns} rows={rows} />
        </div>
      </div>
    </AppShell>
  );
