...
registerPage({
  ...
  path: "/partner/[partnerId]/hrms/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/hrms/admin/page.tsx",
});

export default async function HrmsAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("hrms");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "HRMS / Payroll"} · Admin`}>
      <SuperAdminGate>
        <div>
