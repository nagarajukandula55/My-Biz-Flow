...
registerPage({
  ...
  path: "/partner/[partnerId]/loyalty-rewards/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  explanation: "No-code configuration screen for the loyalty-rewards module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/partner/[partnerId]/loyalty-rewards/admin/page.tsx",
});

export default async function LoyaltyRewardsAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("loyalty-rewards");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Loyalty & Rewards"} · Admin`}>
      <SuperAdminGate>
        <div>
