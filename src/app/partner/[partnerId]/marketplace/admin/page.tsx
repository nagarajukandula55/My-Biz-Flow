...
registerPage({
  ...
  title: "Marketplace / Partner Aggregator — Admin",
  path: "/partner/[partnerId]/marketplace/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/marketplace/admin/page.tsx",
});

export default async function MarketplaceAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("marketplace");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Marketplace / Partner Aggregator"} · Admin`}>
      <SuperAdminGate>
        <div>
