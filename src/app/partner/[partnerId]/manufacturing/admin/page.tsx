...
registerPage({
  ...
  path: "/partner/[partnerId]/manufacturing/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/manufacturing/admin/page.tsx",
});

export default async function ManufacturingAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("manufacturing");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Manufacturing"} · Admin`}>
      <SuperAdminGate>
        <div>
