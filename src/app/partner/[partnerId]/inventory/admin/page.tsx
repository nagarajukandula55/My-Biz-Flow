...
registerPage({
  ...
  path: "/partner/[partnerId]/inventory/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  explanation: "No-code configuration screen for the inventory module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/partner/[partnerId]/inventory/admin/page.tsx",
});

export default async function InventoryAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("inventory");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Inventory"} · Admin`}>
      <SuperAdminGate>
        <div>
