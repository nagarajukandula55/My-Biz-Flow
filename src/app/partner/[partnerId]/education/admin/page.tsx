...
registerPage({
  ...
  path: "/partner/[partnerId]/education/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  explanation: "No-code configuration screen for the education module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/partner/[partnerId]/education/admin/page.tsx",
});

export default async function EducationAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("education");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Education / Coaching"} · Admin`}>
      <SuperAdminGate>
        <div>
