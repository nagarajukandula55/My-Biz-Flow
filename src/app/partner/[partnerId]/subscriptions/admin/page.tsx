...
registerPage({
  ...
  path: "/partner/[partnerId]/subscriptions/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/subscriptions/admin/page.tsx",
});

export default async function SubscriptionsAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("subscriptions");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Subscriptions / Membership"} · Admin`}>
      <SuperAdminGate>
        <div>
