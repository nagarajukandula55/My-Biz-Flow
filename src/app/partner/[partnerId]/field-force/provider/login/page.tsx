...
import { loginProviderAction } from "@/lib/fieldForce/actions";
import { getLocaleFromCookie } from "@/lib/i18n/cookie";
import { t, isRtl } from "@/lib/i18n/locales";
import { LanguageSwitcher } from "@/components/LanguageSwitcher";

registerPage({
  id: "field-force.provider.login",
  moduleSlug: "field-force",
  title: "Field Force — Provider Login",
  path: "/partner/[partnerId]/field-force/provider/login",
  kind: "form",
  superAdminOnly: false,
  customizableRegions: [],
  explanation: "Public login page for a self-signed-up Provider — verifies phone+password, sets the mbf_ff_provider_session cookie.",
  sourceFile: "src/app/partner/[partnerId]/field-force/provider/login/page.tsx",
});
...
export default function ProviderLoginPage({
  ...
  const action = loginProviderAction.bind(null, params.partnerId);
  ...
  return (
    <div dir={isRtl(locale) ? "rtl" : "ltr"} className="mx-auto max-w-md p-6">
      ...
      <p className="mt-4 text-sm text-text-muted">
        ...
        <Link href={`/partner/${params.partnerId}/field-force/provider/signup`} className="text-teal hover:underline">
