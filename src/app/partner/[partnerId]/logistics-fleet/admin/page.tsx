...
registerPage({
  ...
  path: "/partner/[partnerId]/logistics-fleet/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/logistics-fleet/admin/page.tsx",
});

export default async function LogisticsFleetAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("logistics-fleet");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Logistics / Fleet"} · Admin`}>
      <SuperAdminGate>
        <div>
