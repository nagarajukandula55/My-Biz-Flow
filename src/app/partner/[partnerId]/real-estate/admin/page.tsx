...
registerPage({
  ...
  path: "/partner/[partnerId]/real-estate/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  explanation: "No-code configuration screen for the real-estate module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/partner/[partnerId]/real-estate/admin/page.tsx",
});

export default async function RealEstateAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("real-estate");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Real Estate"} · Admin`}>
      <SuperAdminGate>
        <div>
