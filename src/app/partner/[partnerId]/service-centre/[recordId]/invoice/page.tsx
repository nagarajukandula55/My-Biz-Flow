...
registerPage({
  ...
  explanation:
    "Renders a closed Service Centre workorder's Parts & Service Lines as a billable GST Sales Invoice — A4/A5 print only, no thermal-receipt format. General layout shape (letterhead + meta box with the originating workorder number, Bill To and Payment boxes, itemized GST table, HSN summary, totals box, bank details, signatures, declaration) references AN-CRM's invoice per CLAUDE.md's documented UX-pattern exception; built from scratch against this repo's own design tokens. Tax is split by place of supply: a customer in the partner's own state is intra-state and taxed CGST + SGST at half the slab each, a customer elsewhere is inter-state and taxed IGST at the full slab. A B2C document carrying no tax at all (e.g. a fully non-chargeable warranty job) prints as a plain BILL rather than a TAX INVOICE. Line items derive from this partner's own live BOM materials, not the global sample catalog.",
