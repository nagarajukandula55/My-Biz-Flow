import { AppShell } from "@/components/AppShell";
import { registerPage } from "@/lib/designer/registry";
import { ScBrandClientTable } from "./ScBrandClientTable";
import { ScBrandNewButton } from "./ScBrandNewButton";
import { applyCustomizations } from "@/lib/designer/customizations";
import { scBrandColumns } from "@/lib/sample-data/service-centre-brands";
import { listBusinessRecords } from "@/lib/businessRecords";

registerPage({
  id: "service-centre.brands.list",
  moduleSlug: "service-centre",
  title: "Device Brands — List",
  path: "/partner/[partnerId]/service-centre/brands",
  kind: "list",
  superAdminOnly: false,
  customizableRegions: [
    { key: "columns", label: "Table columns" },
    { key: "filters", label: "List filters" },
  ],
  explanation: "Partner-owned brand catalog for the service-centre module — used when creating a workorder.",
  sourceFile: "src/app/partner/[partnerId]/service-centre/brands/page.tsx",
});

export const dynamic = "force-dynamic";

export default async function ScBrandListPage({ params }: { params: { partnerId: string } }) {
  const columns = await applyCustomizations("service-centre.brands.list", scBrandColumns);
  const rows = await listBusinessRecords(params.partnerId, "service-centre-brands");

  return (
    <AppShell topbarTitle="Device Brands" topbarActions={<ScBrandNewButton partnerId={params.partnerId} />}>
      <div>
        <div className="mt-2">
          <ScBrandClientTable partnerId={params.partnerId} columns={columns} rows={rows} />
        </div>
      </div>
    </AppShell>
  );
}
