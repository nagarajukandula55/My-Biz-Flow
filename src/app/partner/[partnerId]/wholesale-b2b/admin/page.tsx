...
registerPage({
  ...
  path: "/partner/[partnerId]/wholesale-b2b/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/wholesale-b2b/admin/page.tsx",
});

export default async function WholesaleB2bAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("wholesale-b2b");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Wholesale / Distributor B2B"} · Admin`}>
      <SuperAdminGate>
        <div>
