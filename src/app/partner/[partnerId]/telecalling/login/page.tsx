...
registerPage({
  ...
  customizableRegions: [],
  explanation:
    "Independent login for a Telecaller agent (PartnerStaff, role=Telecaller) — separate from the business owner's own /login. Verifies email+password, sets the staff session cookie (src/lib/partnerSession.ts), and is the ONLY page under /partner/[partnerId]/* reachable with no session at all (see PartnerLayout's STAFF_AUTH_ROUTE_SUFFIXES).",
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-bg px-4">
      <div className="w-full max-w-sm rounded-lg border border-border bg-bg-raised p-6">
        ...
        <p className="mt-1 text-sm text-text-muted">Sign in with the email and password your manager gave you.</p>
