...
registerPage({
  ...
  path: "/partner/[partnerId]/restaurant-pos/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/restaurant-pos/admin/page.tsx",
});

export default async function RestaurantPosAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("restaurant-pos");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Restaurant POS"} · Admin`}>
      <SuperAdminGate>
        <div>
