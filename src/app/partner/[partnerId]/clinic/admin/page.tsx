...
registerPage({
  ...
  moduleSlug: "clinic",
  title: "Clinic — Admin",
  path: "/partner/[partnerId]/clinic/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  explanation: "No-code configuration screen for the clinic module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/partner/[partnerId]/clinic/admin/page.tsx",
});

export default async function ClinicAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("clinic");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Clinic"} · Admin`}>
      <SuperAdminGate>
        <div>
