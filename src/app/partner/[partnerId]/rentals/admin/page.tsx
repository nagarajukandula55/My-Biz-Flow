...
registerPage({
  ...
  path: "/partner/[partnerId]/rentals/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/rentals/admin/page.tsx",
});

export default async function RentalsAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("rentals");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Rentals / Booking"} · Admin`}>
      <SuperAdminGate>
        <div>
