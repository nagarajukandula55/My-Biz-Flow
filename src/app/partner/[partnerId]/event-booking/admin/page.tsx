...
registerPage({
  ...
  path: "/partner/[partnerId]/event-booking/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/event-booking/admin/page.tsx",
});

export default async function EventBookingAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("event-booking");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Event / Venue Booking"} · Admin`}>
      <SuperAdminGate>
        <div>
