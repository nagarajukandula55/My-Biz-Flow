...
registerPage({
  ...
  path: "/partner/[partnerId]/amc-field-service/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/amc-field-service/admin/page.tsx",
});

export default async function AmcFieldServiceAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("amc-field-service");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "AMC / Field Service"} · Admin`}>
      <SuperAdminGate>
        <div>
