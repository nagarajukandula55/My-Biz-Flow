...
registerPage({
  ...
  path: "/partner/[partnerId]/accounting-gst/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/accounting-gst/admin/page.tsx",
});

export default async function AccountingGstAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("accounting-gst");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "Accounting / GST Compliance"} · Admin`}>
      <SuperAdminGate>
        <div>
