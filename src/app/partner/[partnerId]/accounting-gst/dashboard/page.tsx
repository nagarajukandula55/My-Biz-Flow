...
registerPage({
  ...
  title: "GST — Dashboard",
  path: "/partner/[partnerId]/accounting-gst/dashboard",
  kind: "dashboard",
  ...
  sourceFile: "src/app/partner/[partnerId]/accounting-gst/dashboard/page.tsx",
});
...
export default async function GstDashboardPage({
  ...
  const [invoices, itcEntries, returns] = await Promise.all([
    ...
    listBusinessRecords(params.partnerId, "accounting-gst-itc"),
    listBusinessRecords(params.partnerId, "accounting-gst"),
  ]);
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-border bg-bg-raised p-4">
            ...
            <div className="mt-3 space-y-2 text-sm">
              <FilingRow
                ...
                href={`/partner/${params.partnerId}/accounting-gst/generate?period=${period}&returnType=GSTR-1`}
              />
              <FilingRow
                label="GSTR-3B (Summary Return)"
                dueDate={dueDates.gstr3b}
                status={gstr3b ? String(gstr3b["filingStatus"]) : "Not Generated"}
                href={`/partner/${params.partnerId}/accounting-gst/generate?period=${period}&returnType=GSTR-3B`}
              />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-border bg-bg-raised p-4">
            ...
            <div className="mt-3 flex flex-col gap-2 text-sm">
              <Link href={`/partner/${params.partnerId}/accounting-gst/hsn-summary`} className="btn-outline text-left">
                HSN-wise Summary
              </Link>
              <Link href={`/partner/${params.partnerId}/accounting-gst/itc`} className="btn-outline text-left">
                ITC Register
              </Link>
              <Link href={`/partner/${params.partnerId}/accounting-gst`} className="btn-outline text-left">
                All Filed Returns
              </Link>
            </div>
          </div>
