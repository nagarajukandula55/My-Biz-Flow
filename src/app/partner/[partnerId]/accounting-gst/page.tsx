...
registerPage({
  ...
  path: "/partner/[partnerId]/accounting-gst",
  kind: "list",
  ...
  sourceFile: "src/app/partner/[partnerId]/accounting-gst/page.tsx",
});

export const dynamic = "force-dynamic";

export default async function AccountingGstPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("accounting-gst");
  const columns = await applyCustomizations("accounting-gst.list", accountingGstColumns);
  const rows = await listBusinessRecords(params.partnerId, "accounting-gst");
    ...
    <AppShell
      ...
      topbarActions={
        <div className="flex items-center gap-2">
          <Link href={`/partner/${params.partnerId}/accounting-gst/dashboard`} className="btn-outline">
            Dashboard
          </Link>
          <Link href={`/partner/${params.partnerId}/accounting-gst/generate`} className="btn-outline">
            Generate Return
          </Link>
          <AccountingGstNewButton partnerId={params.partnerId} />
        </div>
      }
    >
      <div>
        ...
        <div className="mt-6">
          <AccountingGstClientTable partnerId={params.partnerId} columns={columns} rows={rows} />
        </div>
      </div>
    </AppShell>
  );
