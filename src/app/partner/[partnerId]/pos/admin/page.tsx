...
registerPage({
  ...
  moduleSlug: "pos",
  title: "POS — Admin",
  path: "/partner/[partnerId]/pos/admin",
  kind: "admin",
  superAdminOnly: true,
  customizableRegions: [
  ...
  sourceFile: "src/app/partner/[partnerId]/pos/admin/page.tsx",
});

export default async function PosAdminPage({ params }: { params: { partnerId: string } }) {
  const mod = await getModule("pos");

  return (
    <AppShell topbarTitle={`${mod?.label ?? "POS"} · Admin`}>
      <SuperAdminGate>
        <div>
