...
registerPage({
  ...
  explanation: "No-code configuration screen for the service-centre module (Super Admin only): custom field definitions, pipeline/workflow stages, and role permissions for this module.",
  sourceFile: "src/app/vendor/[vendorId]/service-centre/admin/page.tsx",
});

export default async function ServiceCentreAdminPage({ params }: { params: { vendorId: string } }) {
  const mod = await getModule("service-centre");

  return (
    <AppShell vendorId={params.vendorId} navGroups={await buildVendorAdminNavGroups(undefined, "service-centre")} topbarTitle={`${mod?.label ?? "Service Centre"} · Admin`}>
      <SuperAdminGate>
        <div>
