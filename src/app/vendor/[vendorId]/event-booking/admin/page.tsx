...
export default async function EventBookingAdminPage({ params }: { params: { vendorId: string } }) {
  const mod = await getModule("event-booking");

  return (
    <AppShell vendorId={params.vendorId} navGroups={await buildVendorAdminNavGroups(undefined, "event-booking")} topbarTitle={`${mod?.label ?? "Event / Venue Booking"} · Admin`}>
      <SuperAdminGate>
        <div>
