...
export default async function NewMarketplacePage({ params }: { params: { vendorId: string } }) {
  ...
  return (
    <AppShell topbarTitle={`New Vendor Listing — ${mod?.label ?? "Marketplace / Vendor Aggregator"}`}>
      <div>
        ...
        <div className="mt-6">
          <RecordForm
            ...
            action={createBusinessRecordAction.bind(null, params.vendorId, "marketplace")}
          />
        </div>
      </div>
    </AppShell>
